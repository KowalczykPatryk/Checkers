"""
Value Network that evalues position on the board when final state in the simulation step
is not reached.
"""
...
class ValueNetwork(torch.nn.Module):
    """
    Is used to evaluate the position on the board if max depth of the search
    was reached and game is not finished.
    """
    def __init__(self, in_channels: int, n_classes: int = 2) -> None:
        ...
        # input size = 10
        self.conv1 = torch.nn.Conv2d(in_channels=in_channels, out_channels=8, kernel_size=3,
                                     padding=1)
